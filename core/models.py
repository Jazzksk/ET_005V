...
class Producto(models.Model):
    ...
    imagen = models.FileField(upload_to='img/productos')
    descripcion = models.CharField(max_length=200, null=False)
    stock = models.IntegerField(default=0, verbose_name='Stock') #Aqui estoy agregando el stock al modelo
...
class Suscripcion(models.Model):
    usuario = models.ForeignKey(User, on_delete=models.CASCADE, related_name="suscripción")
...
class Compra(models.Model):
    usuario = models.ForeignKey(User, on_delete=models.CASCADE, related_name="compra")
    total = models.DecimalField(null=False,max_digits=10,decimal_places=2)
...
class Compra_item(models.Model):
    producto = models.ForeignKey(Producto, on_delete=models.CASCADE)
    compra = models.ForeignKey(User, on_delete=models.CASCADE, related_name="item_compra")

    def __str__(self) -> str:
        return f"Id: {self.pk} | Producto: {self.producto.titulo} |  Compra_id {self.compra.id}"
